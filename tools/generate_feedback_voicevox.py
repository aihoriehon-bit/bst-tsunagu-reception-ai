...
OUT = ROOT / 'wide-desk-feedback/audio'

def spoken_text(text):
    # Resolve ambiguous kanji in the speech input; keep the screen's text intact.
    for source, reading in (
        ('AI', 'エーアイ'), ('取次ぎ', 'とりつぎ'),
        ('社員の方', '社員のかた'), ('お越しになった方', 'お越しになったかた'),
        ('お話しください', 'おはなしください'),
        ('行っていません', 'おこなっていません'),
    ):
...
def main():
    ...
    lines = json.loads((ROOT / 'wide-desk-feedback/dialogue-lines.json').read_text())
