...
export function bindNameAvailability({ nameInput, readingInput, panel }) {
  const composing = new Set();
  const refresh = () => {
    if (composing.size) return;
    const result = nameAvailability({ name: nameInput.value, reading: readingInput.value });
  ...
  for (const field of [nameInput, readingInput]) {
