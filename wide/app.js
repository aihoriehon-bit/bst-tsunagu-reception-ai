...
const FACE_DB_STORAGE_KEY = "tsunagu-face-db-v2";
...
function drawFaceTexture(speaking, blinkFrame, elapsed) {
  ...
  const frameKey = `${state.faceAssetsReady}-${speaking}-${blinkFrame}-${mouthFrame}-${state.winkSide || ""}`;
...
function getEyeAsset(side, blinkFrame) {
  ...
  // ウインク中は指定した側だけ閉じ、反対の目は開いたままにする
  if (state.winkSide && side !== state.winkSide) return state.faceAssets[`${prefix}Open`];
...
function setupBlinking() {
  const blink = () => {
    // 接客中はたまに片目だけ閉じてウインク（理想動画のラストの表情）
    state.winkSide =
      state.behavior === "attending" && Math.random() < 0.15
        ? Math.random() < 0.5
          ? "left"
          : "right"
        : null;
    state.blinkDuration = state.winkSide ? 480 : 280;
...
async function computeFaceDescriptor() {
  ...
  try {
    // 離れて立つ来客の小さい顔も拾えるよう、入力サイズは大きめ・検出しきい値は低めにする
...
function findBestFaceMatch(descriptor) {
  ...
  state.faceDb.forEach((person) => {
    let personBest = Infinity;
    person.descriptors.forEach((sample) => {
      personBest = Math.min(personBest, euclideanDistance(descriptor, sample));
    });
    if (person.descriptors.length > 1) {
      personBest = Math.min(personBest, euclideanDistance(descriptor, faceCentroid(person)));
    }
    if (!best || personBest < best.distance) best = { name: person.name, distance: personBest };
  });
  return best;
}
...
async function identifyVisitor(attempts = 5) {
    ...
    for (let i = 0; i < attempts; i += 1) {
      ...
      if (state.faceDb.length) {
        ...
        if (descriptor) {
          ...
          if (best) {
            console.info(
              `[顔照合] 一番近い: ${best.name} 距離${best.distance.toFixed(3)}（しきい値${FACE_MATCH_THRESHOLD}）`,
            );
            if (best.distance <= FACE_MATCH_THRESHOLD) return { name: best.name, kind: "face" };
          }
        }
      }
      // 顔で分からなければ服装（制服）で判定
  ...
  state.matchTestTimer = window.setInterval(async () => {
    ...
    if (state.faceDb.length) {
      ...
      if (!descriptor) {
        ...
        const best = findBestFaceMatch(descriptor);
        if (best) {
          const ok = best.distance <= FACE_MATCH_THRESHOLD;
          anyMatch = anyMatch || ok;
          parts.push(
            `顔: ${best.name}（距離 ${best.distance.toFixed(2)} / ${FACE_MATCH_THRESHOLD}）→ ${ok ? "本人" : "別人"}`,
          );
        }
      }
    }
