...
export function loadKanaBank() {
  if (!bankPromise) bankPromise = (async () => {
    const responses = await Promise.all(['bank.json', 'bank.wav'].map(file => fetch(new URL('../wide-desk-preview/audio/kana/' + file + '?v=3', import.meta.url), { signal: AbortSignal.timeout(12000) })));
