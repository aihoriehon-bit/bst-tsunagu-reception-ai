...
Object.assign(texts, GUIDED_TEXTS);
export const DIALOGUE_LINES = Object.fromEntries(Object.entries(texts).map(([key, text]) => [key, {
  text, audio: `../wide-desk-feedback/${key in GUIDED_TEXTS ? 'guided-audio' : 'audio'}/${key}.wav`, group: 'attend',
}]));
...
export function createConversation({ onSpeak, onInterrupt, onEnableAudio, onRestart, available, registeredNames = () => [] }) {
  ...
  function renderCue() {
    ...
    turnIndicator.show(cue, { visible, host: confirmation.cueHost || choicePanel.cueHost });
    choicePanel.setCue(cue);
  ...
  return {
    ...
    repeatPrompt() {
      if (!active || completed || !present || document.hidden || !audible) return;
      const own = epoch;
