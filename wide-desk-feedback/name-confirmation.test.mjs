...
test('actual name generator calls unreviewed registered names and honors explicit OFF', async () => {
  ...
  globalThis.fetch = async url => new Response(readFileSync(new URL(String(url).includes('bank.json') ? '../wide-desk-preview/audio/kana/bank.json' : '../wide-desk-preview/audio/kana/bank.wav', import.meta.url)));
  try {
    ...
    assert.match(trial.audio, /^\.\.\/wide-desk-preview\/audio\/name-bank\/.*\.mp3$/); assert.equal(trial.spokenText, undefined);
    ...
    const sato = { name: '佐藤' };
    assert.equal((await nameLine({ ...sato, nameAudioApproval: nameApprovalToken(sato) })).audio, '../wide-desk-preview/audio/nameSato.wav');
