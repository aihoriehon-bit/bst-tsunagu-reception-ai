...
test('body-only presence requires three separate positive frames, and resets on departure/gaps', () => {
  ...
  gate.update(1, 4000); assert.equal(gate.update(1, 10500), 0);
  gate.reset(); assert.equal(gate.update(1, 11000), 0);
});
test('actual identity gate rejects body-only, tiny faces, stale frames and a second body', () => {
  ...
  const state = { faceQuality, frameLifetime, detectionMs: 0, peopleCount: 1, faces: [], facesAt: Date.now(), video: { readyState: 2, videoWidth: 1920, videoHeight: 1080 } };
