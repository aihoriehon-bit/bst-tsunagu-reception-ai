...
import { NAME_RECORDINGS as BANK_RECORDINGS } from './name-library-data.mjs';
import { nameLine } from './name-voice.js';

test('5000 surname and 5000 given readings share complete verified MP3 recordings', () => {
  const catalog = JSON.parse(readFileSync(new URL('../wide-desk-preview/audio/name-bank/catalog.json', import.meta.url)));
  ...
  assert.equal(BANK_RECORDINGS.length, readings.size);
  ...
  for (const entry of BANK_RECORDINGS) {
    ...
    assert.match(entry.audio, /^\.\.\/wide-desk-preview\/audio\/name-bank\/n-[a-f0-9-]+\.mp3$/);
  ...
  const old = JSON.parse(readFileSync(new URL('../wide-desk-preview/audio/names/catalog.json', import.meta.url)));
