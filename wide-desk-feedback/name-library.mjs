import { NAME_RECORDINGS as BANK_RECORDINGS } from './name-library-data.mjs?v=20260910-bank10000-1';
import { normalizeReading } from './kana-name.mjs?v=20260909-5';

export const NAME_RECORDINGS = [...BANK_RECORDINGS, {
  reading: 'まっきぃ', audio: '../wide-desk-feedback/audio/nameMakkii.mp3',
  names: ['まっきぃ', 'まっきい', 'まっきー', 'マッキィ', 'マッキイ', 'マッキー'],
  categories: ['nickname'],
}];
const byReading = new Map(NAME_RECORDINGS.map(entry => [entry.reading, entry]));
for (const alias of ['まっきい', 'まっきー']) byReading.set(alias, byReading.get('まっきぃ'));
