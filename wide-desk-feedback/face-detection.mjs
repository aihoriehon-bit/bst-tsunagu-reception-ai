...
export function mergeDetections(detections) {
  ...
  for (const d of [...detections].sort((a, b) => (b.score || 0) - (a.score || 0))) {
    ...
    const duplicate = result.some(previous => {
      const b = previous.boundingBox;
      ...
      const large = Math.max(a.width * a.height, b.width * b.height);
      const sameCenter = Math.abs(a.originX + a.width / 2 - b.originX - b.width / 2) < Math.max(a.width, b.width) * .15
        && Math.abs(a.originY + a.height / 2 - b.originY - b.height / 2) < Math.max(a.height, b.height) * .15;
      // Only add the containment rule across different crops of the SAME frame.
      // Two separately found faces in one crop remain separate even if one is small.
      const crossCrop = d.sourceRegion !== undefined && previous.sourceRegion !== undefined
        && d.sourceRegion !== previous.sourceRegion && d.capturedAt === previous.capturedAt;
      return intersection / (a.width * a.height + b.width * b.height - intersection) > .4
        || (crossCrop && sameCenter && small / large >= .2 && intersection / small > .9);
    });
...
export async function detectFaces(api, video, { frame, tile } = {}, area = null, focusArea = null) {
  ...
  const regions = searchRegions(area.width, area.height).map(local => ({ ...local, x: local.x + area.x, y: local.y + area.y }));
  if (focusArea && !regions.some(r => ['x', 'y', 'width', 'height'].every(k => r[k] === focusArea[k]))) regions.push(focusArea);
  for (const region of regions) {
    ...
    all.push(...detections.filter(d => [d.box.x, d.box.y, d.box.width, d.box.height].every(Number.isFinite)
      && d.box.width > 0 && d.box.height > 0).map(d => ({ capturedAt, sourceRegion: `${region.x},${region.y},${region.width},${region.height}`, score: d.score, boundingBox: {
