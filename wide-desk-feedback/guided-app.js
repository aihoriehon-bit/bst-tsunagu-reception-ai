...
const TEST_VISITOR_MS = 180000; // 選択式の受付を最後まで試せるよう3分間。
...
// Delivery uses a dedicated handover guide; visits collect the addressee.
SPEECH_LINES.calling = DIALOGUE_LINES.chatDelivery;
SPEECH_LINES.visitor = DIALOGUE_LINES.chatGuestRecipient;
// Real employees use their registered name separately. Demos must not invent
// a person's name or automatically begin a visitor intake either.
for (const key of ['employeeGeneric', 'employeeSato', 'employeeTanaka']) {
  SPEECH_LINES[key] = { ...DIALOGUE_LINES.chatRecognizedEmployee, group: 'employee' };
}
...
const conversation = createConversation({
  ...
  onRestart() { return beginSensorGreeting(); },
...
async function speakLine(speechKey, button = null, { onFinish = null, onFailure = null, keepPose = false, standForSpeech = false } = {}) {
  ...
  if (line.audio) audio.src = line.audio.startsWith('blob:') ? line.audio : `${line.audio}?v=${SPEECH_AUDIO_VERSION}`;
  audio.preload = "auto";
  audio.volume = 1;
  ...
  if (!line.audio && line.spokenText) {
    deviceVoiceTimeout = setTimeout(() => { cancelNameVoice(); finish(true); }, 15000);
    speakDeviceName(line.spokenText, { onstart: startLipSync, onend: () => finish(), onerror: () => finish(true) });
    return;
  }
...
function startDetectionLoop() {
  ...
  cameraDetectionLoop ??= createDetectionLoop({
    ...
    detect: async () => {
      ...
      return { faces, people: personDetector.detect(cameraPreviewElement, visitorRecognition.region), bodyBoxes: personDetector.boxes, faceError };
    },
    update: ({ faces = [], people = 0, bodyBoxes = [], faceError = false } = {}) => {
      visitorRecognition.updatePeople(people, bodyBoxes);
  ...
  try {
    ...
    if (soundEnabled) {
      ...
      const greet = () => {
          ...
          if (!previousPlan.identity) followUp = conversation.recognizeVisitor(person) || followUp;
          const followUpSequence = sequenceId + 1;
          const finish = () => { if (sequenceId === followUpSequence) greetingPending = false; };
          speakLine(followUp, null, { onFinish() { finish(); if (!previousPlan.identity) conversation.repeatPrompt(); }, onFailure: finish });
      };
...
async function beginSensorGreeting() {
  ...
  if (currentReceptionPlan.identity) {
    ...
    for (const person of companions) {
      ...
      const greetingIndex = currentReceptionPlan.greeting.findIndex(k => !k.startsWith('registeredName'));
      currentReceptionPlan.greeting.splice(greetingIndex < 0 ? 0 : greetingIndex,0,key);
    }
  }
  pendingTestSpeechKey = null;
...
function returnToDeskAfterStartup() {
  if (!mixer || conversation.active || sensorAttending || isSensorVisitorPresent()) return;
  const ownSequence = ++sequenceId;
  playMotion("sitDown");
  setActiveButton("sitDown");
  window.setTimeout(() => {
